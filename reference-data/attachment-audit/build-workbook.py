"""Build the human-readable workbook from the canonical attachment reference JSON.

The workbook and JSON are optional reference material, not live site inputs.
Future additions should be visually reviewed before promotion into ``data/``.

    python reference-data/attachment-audit/build-workbook.py [--json PATH] [--out PATH]
"""
...
DEFAULT_JSON = HERE / "attachment-screenshot-review.json"
DEFAULT_OUT = HERE / "BF6_Attachment_Stats_Review.xlsx"
CAPTURE_ROOT = HERE / "Weapon Attachments"
...
def _file_url(path):
    ...
    Resolve the portable ``Weapon Attachments`` suffix against the screenshot folder beside
    this script before creating the workbook hyperlink.
    """
    ...
    try:
        ...
        resolved = CAPTURE_ROOT.joinpath(*parts[index + 1:]).resolve()
