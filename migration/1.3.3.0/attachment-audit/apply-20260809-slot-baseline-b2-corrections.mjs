...
const ledgerPath = path.join(auditRoot, 'slot-baseline-evidence-b2-20260809.json');
...
if (ledger.metadata?.stage !== 'B2'
  || ledger.metadata?.selection?.groupRankMin !== 31
  || ledger.metadata?.selection?.groupRankMax !== 60
  || ledger.metadata?.selection?.groupCount !== 30
  || ledger.metadata?.selection?.affectedViolationCount !== 43
  || ledger.metadata?.selection?.readingCount !== 73) {
  throw new Error('B2 ledger metadata does not match the requested scope');
}
if (!Array.isArray(ledger.entries) || ledger.entries.length !== 73) {
  throw new Error(`Expected exactly 73 B2 ledger entries; found ${ledger.entries?.length ?? 'non-array'}`);
}

const corrections = ledger.entries.filter(entry => entry.disposition === 'corrected');
if (corrections.length !== 46) {
  throw new Error(`Expected exactly 46 corrected B2 ledger entries; found ${corrections.length}`);
}
if (ledger.entries.some(entry => entry.observedArrow !== null)) {
  throw new Error('B2 ledger contains a non-null observed arrow');
}
