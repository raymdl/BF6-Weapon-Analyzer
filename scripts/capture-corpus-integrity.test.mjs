...
const inputs = loadCaptureCorpus(DEFAULT_ROOT);
...
test('capture corpus fixtures are complete, full-roster, and path-portable', () => {
  ...
  for (const file of [
    'scripts/capture-corpus-lib.mjs',
...
function copyNameEffectInventoryFixture() {
  ...
  const files = new Set([
    ...Object.values(CAPTURE_CORPUS_FIXTURES),
...
test('cross-field consistency checks named capacity and every stat in duplicate identities', () => {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'bf6-corpus-check2-'));
  const fixturePaths = new Set([...Object.values(CAPTURE_CORPUS_FIXTURES), 'data/attachments.json']);
  ...
  const reviewPath = path.join(root, CAPTURE_CORPUS_FIXTURES.audit);
  ...
  const originalReview = JSON.parse(fs.readFileSync(
    path.join(DEFAULT_ROOT, CAPTURE_CORPUS_FIXTURES.audit), 'utf8',
  ));
  try {
...
test('default CLI checks are read-only, work outside the repository cwd, and fail on missing fixtures', () => {
  ...
  const tempCwd = mkdtempSync(path.join(tmpdir(), 'bf6-corpus-cwd-'));
  const missingRoot = mkdtempSync(path.join(tmpdir(), 'bf6-corpus-missing-'));
  try {
    ...
    assert.throws(() => runSweep({ root: missingRoot }), /Missing required capture corpus fixture/);
    assert.throws(() => runFieldSlotDiscovery({ root: missingRoot }), /Missing required capture corpus fixture/);
  } finally {
