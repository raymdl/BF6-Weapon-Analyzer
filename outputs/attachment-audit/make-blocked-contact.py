...
doc = json.load(open(os.path.join(root, 'outputs', 'attachment-audit', 'attachment-screenshot-review.json'), encoding='utf-8'))
man = json.load(open(os.path.join(root, 'outputs', 'attachment-audit', 'rename-manifest.json'), encoding='utf-8'))
...
for weapon in sorted({e['weaponName'] for e in items}):
    ...
    out.save(os.path.join(root,'outputs','attachment-audit',f'blocked-{weapon.replace(" ","_")}.jpg'),quality=85)
