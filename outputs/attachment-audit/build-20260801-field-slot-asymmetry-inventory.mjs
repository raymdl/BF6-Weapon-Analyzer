...
export const OUTPUT_RELATIVE_PATH = 'outputs/attachment-audit/field-slot-asymmetry-inventory-20260801.json';
export const GENERATED_BY = 'outputs/attachment-audit/build-20260801-field-slot-asymmetry-inventory.mjs';
...
export function deriveCorpusInventory(audit) {
  ...
  return { slotsByField, examplesByKey, evidenceByKey };
}
...
function sourceFor(field, slot, classification) {
  if (classification === 'known-deferral') {
    if (field === 'sprintRecoveryMs' && slot === 'Barrel') return 'docs/BARREL_VELOCITY_PHASE7_INVENTORY.md (§ Sprint-recovery anomalies (deferred))';
    if (field === 'collateralMultiplier' || field === 'sprintRecoveryMs') return 'outputs/attachment-audit/sl9-detailed-recapture-20260731.json';
    return 'outputs/attachment-audit/subsonic-velocity-treatments-20260731.json; data/ammo.json (no selectable subsonic ammo type)';
  }
...
function buildInventory(root = DEFAULT_ROOT) {
  ...
  for (const field of STATS) {
    ...
    for (const slot of asymmetricalSlots) {
      ...
      const adjudicatedGrip = classification === 'confirmed-corpus-error'
        ? corpusEvidence.find(item => item.sourcePath === 'Weapon Attachments/Shotgun/18.5KS-K/23_18.5KS-K_Grip_Alloy_Vertical.png')
        : null;
      if (classification === 'confirmed-corpus-error') assert.ok(adjudicatedGrip);
  ...
  return {
    ...
    schemaVersion: 1,
    source: 'outputs/attachment-audit/attachment-screenshot-review.json plus current resolver behavior',
