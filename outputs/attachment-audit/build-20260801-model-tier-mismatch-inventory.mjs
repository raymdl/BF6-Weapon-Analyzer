...
const AUDIT_PATH = path.join(ROOT, 'outputs', 'attachment-audit', 'attachment-screenshot-review.json');
const OUTPUT_PATH = path.join(ROOT, 'outputs', 'attachment-audit', 'model-tier-mismatch-inventory-20260801.json');
...
assert.equal(records.length, 74, `expected 74 mismatch findings, found ${records.length}`);
assert.equal(records.filter(record => record.adjudication !== 'unadjudicated').length, 6);
assert.equal(records.filter(record => record.adjudication === 'site-bug').length, 4);
assert.equal(records.filter(record => record.adjudication === 'corpus-error').length, 2);
assert.equal(records.filter(record => record.adjudication === 'unadjudicated').length, 68);

const inventory = {
  kind: 'model-tier-mismatch-inventory',
  schemaVersion: 1,
  source: 'outputs/attachment-audit/attachment-screenshot-review.json',
  generatedBy: 'outputs/attachment-audit/build-20260801-model-tier-mismatch-inventory.mjs',
  policy: '§7 screenshot wins; disagreements are reported and never resolved by rewriting the corpus or data.',
  counts: { total: 74, siteBug: 4, corpusError: 2, unadjudicated: 68 },
  records,
};
