...
- **Temporary status:** PP-19 now uses the user-confirmed PW5A3 model with community-tested automatic tier values. The checks below remain required for authoritative game-file/breakpoint confirmation, but they no longer block Luna C from auditing attachments.
