...
Implementation: [sim/damage.js](../sim/damage.js), [sim/core.js](../sim/core.js),
[sim/ballistics.js](../sim/ballistics.js), [sim/target.js](../sim/target.js), and
chart/projectile assembly in [ui/app.js](../ui/app.js). The equations and validation
limits below describe these modules.
...
The generated map covers all 63 weapons and 328 supported ammo choices.
`resolveHitMultipliers()` requires the selected weapon/ammo record and finite
multipliers. Null ammo selects the weapon's explicit base record. Missing or
invalid values raise a required-data error; scripts throw by default, while the
browser reports the error and leaves dependent results unavailable.
...
need two. BTK is conditional on the specified hit sequence; it does not estimate player
accuracy.
...
gravity. Source coefficients alone do not validate native trajectories.
...
The 75-damage threshold affects styling only.

In-game accuracy remains unverified for target geometry, sampling, drag, and
source-modifier composition. See [limitations](MODEL_LIMITATIONS.md). Tests check
