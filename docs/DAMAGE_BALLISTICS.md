...
The generated map covers all 63 weapons and 328 supported ammo choices. The
resolver retains a defensive base-value fallback, then legacy 1.34 head/1 limb
defaults if data is absent. Validation requires complete current coverage; these
fallbacks are not the source policy for supported selections.
